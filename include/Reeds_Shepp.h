...
namespace ReedsShepp
{
    ...
    std::vector<Path> generate_path(double sx, double sy, double syaw, double gx, double gy, double gyaw, double maxc, double step_size)
    {
        ...
        for (auto func : path_funcs)
        {
            ...
            if (flag)
                paths = set_path(paths, travels, dirs, step_size);

            std::tie(flag, travels, dirs) = func(-x_, y_, -dth);
            if (flag)
                paths = set_path(paths, timeflip(travels), dirs, step_size);

            std::tie(flag, travels, dirs) = func(x_, -y_, -dth);
            if (flag)
                paths = set_path(paths, travels, reflect(dirs), step_size);

            std::tie(flag, travels, dirs) = func(-x_, -y_, dth);
            if (flag)
                paths = set_path(paths, timeflip(travels), reflect(dirs), step_size);
        }
    ...
    std::tuple<std::vector<double>, std::vector<double>, std::vector<double>, std::string, std::vector<double>> path_planning(double sx, double sy, double syaw, double gx, double gy, double gyaw, double maxc, double step_size)
    {
        auto paths = calc_paths(sx, sy, syaw, gx, gy, gyaw, maxc, step_size);
        if (paths.empty())
            return {{}, {}, {}, "", {}};
        auto min_it = std::min_element(paths.begin(), paths.end(), [](const Path &a, const Path &b)
